...
//                    return true;
        ...
        openFragment(new TextFragment());
//        NewsContentFragment fragment = (NewsContentFragment) getFragmentManager().findFragmentById(R.id.fragment_frame_layout);
...
//
    ...
    //发送蓝牙数据！
    public void sendDataBluetooth(char type) {
        ...
        if (!(currentFragment instanceof BluetoothFragmentInterface))
            return;
        byte[] bytes = ((BluetoothFragmentInterface) currentFragment).getMessage(type);
